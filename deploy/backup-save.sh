...
readonly LOG_MODULE="backup-save"

readonly BACKUP_PATH="$ROOT_PATH/../backup"

backup_save()
{
    local target_pumpos_piu_dir="$1"

    local data_dir="$target_pumpos_piu_dir/data"
...
if [ ! "$1" ]; then
    ...
    echo "Backup all save directories on a target /piu deployment."
    echo "Usage: backup-save <path to /piu folder of mounted pumpos drive>"
    exit 1
fi

target_pumpos_piu_dir="$1"

backup_save "$target_pumpos_piu_dir"