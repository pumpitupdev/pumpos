...
readonly LOG_MODULE="deploy-data"
...
data_deploy_game()
{
    ...
    local target_ver_dir="$target_pumpos_piu_dir/data/$game_ver"
    ...
    if [ -e "$target_ver_dir/piu"  ]; then
        ...
        changelog_append "$target_pumpos_piu_dir/log" "deploy-data" "clean $game_ver"
    fi
    ...
    changelog_append "$target_pumpos_piu_dir/log" "deploy-data" "deployed $game_ver, version $source_data_ver"
}
...
data_deploy()
{
    ...
    verify_file_exists "$target_pumpos_piu_dir/data"
    ...
    if [ "$limit_game_version" ]; then
        ...
        verify_file_exists "$target_pumpos_piu_dir/data/$limit_game_version"
    fi

    if [ ! "$limit_game_version" ]; then
        log_info "Deployment ALL games, source data dir $piu_data_dir, target piu dir $target_pumpos_piu_dir..."
        ...
        log_info "Deployment of SELECTED game $limit_game_version, source data dir $piu_data_dir, target piu dir $target_pumpos_piu_dir..."

        changelog_append "$target_pumpos_piu_dir/log" "deploy-data" "start version $limit_game_version"

        data_deploy_game "$target_pumpos_piu_dir" "$piu_data_dir" "$lib_type" "$limit_game_version"
    fi
...
data_deploy "$target_pumpos_piu_dir" "$data_dir" "$lib_type" "$single_delpoyment"