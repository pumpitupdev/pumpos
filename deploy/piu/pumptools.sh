...
readonly LOG_MODULE="deploy-piu-pumptools"
...
deploy_pumptools_game()
{
    ...
    log_debug "  Deploying pumptools for $game_ver, target dir $target_pumpos_dir..."
    ...
    cp "$hook_file" "$target_pumpos_dir/hook.so"
    cp "$hook_path/$hook_name.conf" "$target_pumpos_dir/hook.conf"
    cp "$hook_path/piueb" "$target_pumpos_dir"
    cp "$piuio_path/ptapi-io-piuio-joystick.so" "$target_pumpos_dir"
    cp "$piuio_path/ptapi-io-piuio-joystick-conf" "$target_pumpos_dir"
    cp "$piuio_path/ptapi-io-piuio-keyboard.so" "$target_pumpos_dir"
    cp "$piuio_path/ptapi-io-piuio-keyboard-conf" "$target_pumpos_dir"
    cp "$piuio_path/ptapi-io-piuio-real.so" "$target_pumpos_dir"
}
...
deploy_pumptool_unpack()
{
	...
	local temp="$target_pumpos_dir/tmp/pt-depl"
...
deploy_pumptools()
{
    ...
    deploy_pumptool_unpack "$target_pumpos_dir" "$pumptools_zip_path"

    local pumptools_unpack_path="$target_pumpos_dir/tmp/pt-depl/pumptools"
    ...
    changelog_append "$target_pumpos_dir/log" "deploy-pumptools" "unpacked $pumptools_version"

    if [ ! "$limit_to_version" ]; then
        log_info "Deployment pumptools ALL games..."

        changelog_append "$target_pumpos_dir/log" "deploy-pumptools" "start all"

        for game_path in "$target_pumpos_dir"/data/*; do
            game_ver="$(basename "$game_path")"

            deploy_pumptools_game "$target_pumpos_dir/data/$game_ver" "$pumptools_unpack_path" "$game_ver"
        done
    else
        log_info "Deployment pumptools SELECTED game $limit_to_version..."

        changelog_append "$target_pumpos_dir/log" "deploy-pumptools" "start version $limit_to_version"

        deploy_pumptools_game "$target_pumpos_dir/data/$limit_to_version" "$pumptools_unpack_path" "$limit_to_version"
    fi

    changelog_append "$target_pumpos_dir/log" "deploy-pumptools" "deployed $pumptools_version"
...
if [ "$#" -lt 2 ]; then
    ...
    echo "Usage: deploy-piu-pumptools <path to /pumpos folder of mounted pumpos drive> <path pumptools.zip> [limit to version]"
    exit 1
fi
...
deploy_pumptools "$target_pumpos_dir" "$pumptools_zip_path" "$limit_to_version"
